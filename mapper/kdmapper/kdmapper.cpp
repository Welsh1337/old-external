...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle)
{
	...
	char* driverbuffer = (char*)&welsh[0];
	int lenght = noac_sys_len;

	std::vector<uint8_t> raw_image(driverbuffer, driverbuffer + lenght);

	/*std::vector<uint8_t> raw_image = { 0 };
	
	if (!utils::ReadFileToMemory("C:\\Users\\vm\\Documents\\Projects\\05-27-2020\\noac\\build\\bin\\noac.sys", &raw_image))
	{
	...
	do
	{
		...
		globalInput.Magic = MAGIC;
		globalInput.Pid = GetCurrentProcessId();
		
		if (!intel_driver::CallKernelFunction(iqvw64e_device_handle, &status, address_of_entry_point, &globalInput))
		{
